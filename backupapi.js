...
function getSavedTeams() {
  getAll().then(function (teams) {
    var teamHTML = '<div class="col s12"><h6 class="title-data">Tim Favorit</h6></div>';

    if (teams.length === 0) {
      teamHTML += '<p class="center-align" style="color: white">Belum ada tim yang tersimpan</p>';
    } else {
      teams.forEach((team) => {
        teamHTML += `
              ...
              <div class="row valign-wrapper">
                  <div class="col s4">
                      <div class="card-image waves-effect waves-block waves-light">
                          <img src="${checkImg(team.crestUrl)}" alt="Logo tim belum tersedia!" class="responsive-img">
                      </div>
                  </div>
  ...
  var articleHTML = `
    <div class="card mt-30 z-depth-0">
      <div class="card-content center-align">
        ...
        <img src="${checkImg(data.crestUrl)}" alt="logo tim belum tersedia!" class="responsive-img mt-30" style="max-height: 150px !important;">

        <p class="mt-30">
          Berdiri pada tahun : ${checkNull(data.founded)}<br>
          Lokasi Stadium : ${checkNull(data.address)}<br>
    ...
    <h6 class="title-data">Daftar Pemain</h6>
    `;
