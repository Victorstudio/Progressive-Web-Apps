...
let textCenter = 0;
const letters = [];
const prompt = ['s', 'e', 'l', 'a', 'm', 'a', 't', ' ', 'd', 'a', 't', 'a', 'n', 'g']; // Tentukan kalimat awal saat pertama kali
let runPrompt = true ;

//pengaturan agar responsive
const resizePage = () => {
...
//pengaturan efek animasi tiap huruf 
...
//pengaturan efek untuk bentuk segitiga
...
//pengaturan efek untuk bentuk lingkaran
const addCirc = (x0, y0) => {
...
//pengaturan efek animasi saat huruf dihapus
...
//pengaturan default huruf saat di input
const onInputChange = () => {
  const value = input.value === '' ? [] : input.value.toLowerCase().split(''); //huruf yang diinput walaupun kecil akan otomatis jadi kapital
  addLetters(value);
  removeLetters(value);
  resizeLetters();
};
